...
- **Linked issue** — scan the branch name, commits, and PR body for identifiers (e.g. `INS-123`, `PROJ-456`). If found, pull it from Linear using the current harness's `get_issue` MCP tool to source the *why*.
