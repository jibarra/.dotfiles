...
Launch the `reviewer-orchestrator` agent as a subagent (the `Task` tool in opencode, the `Agent` tool in Claude Code, or Codex's subagent tool). In the prompt, include:
