...
- **Codebase** — `grep`/`find` via Bash for known symbols; the available exploration subagent (`explore`, `Explore`, or Codex `explorer`) for broad/open-ended searches (3+ queries)
...
- In parallel, spawn the planning subagent — `generic-coding-changes-planner` (opencode or Codex) or the `Plan` agent (Claude Code) — passing the same scope summary, working directory, and any constraints from Step 4.
